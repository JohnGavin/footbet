...
Grid search over sigma_process and sigma_obs, maximising the sum of
log-likelihoods from the Kalman innovation sequence.
...
kalman_tune(
  ...
  sigma_obs_grid = c(0.3, 0.5, 0.7, 1, 1.5),
  use_xg = TRUE
)
```
...
- matches:
  ...
  Tibble of matches (same format as
  [`kalman_strengths()`](https://johngavin.github.io/footbet/reference/kalman_strengths.md)).

- sigma_process_grid:

  Numeric vector. Process noise candidates.

- sigma_obs_grid:

  Numeric vector. Observation noise candidates.
...
A tibble with one row per grid point: `sigma_process`, `sigma_obs`,
`log_lik`.
