...
Queries DuckDB for match results and computes matchday numbers (fixture
round number: ceiling(row_number() / (n_teams / 2)), sorted by date).

## Usage

``` r
oagd_match_data(con, seasons = c("2223", "2324", "2425"), leagues = NULL)
```
...
- con:
  ...
  A DBI connection to the footbet DuckDB.

- seasons:

  Character vector of season codes (e.g. `"2425"`).

- leagues:

  Character vector of league codes. Defaults to all 10.

## Value

A tibble with columns: `match_id`, `season`, `league_code`,
