...
`Y ~ Poisson(lambda2)`, using base R
[`besselI()`](https://rdrr.io/r/base/Bessel.html).

## Usage

``` r
dskellam(k, lambda1, lambda2)
```
...
- k:
  ...
  Integer vector. Goal difference values.

- lambda1:

  Numeric. Home team expected goals (must be \> 0).

- lambda2:

  Numeric. Away team expected goals (must be \> 0).

## Value

Numeric vector of probabilities, same length as `k`.
