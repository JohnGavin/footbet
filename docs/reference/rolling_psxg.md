...
Post-shot xG (PSxG) measures expected goals given where the shot was
placed on the goalframe. PSxG - xG captures finishing quality beyond
shot creation. A positive value means the team places shots better than
the average shooter from those positions.

## Usage

``` r
rolling_psxg(matches_df, window = 5L)
```
...
- matches_df:
  ...
  A tibble with `team`, `match_date`, `psxg` (post-shot xG), `xg`
  (pre-shot xG).

- window:

  Integer. Rolling window size (default 5).

## Value

A tibble with `team`, `match_date`, `rolling_psxg`,
