...
Updates Elo ratings match-by-match using the `elo` package. Matches must
be sorted by date. Returns ratings after each match.

## Usage

``` r
compute_elo(matches_df, k = 20, home_advantage = 65, init = 1500)
```
...
- k:
  ...
  Numeric. K-factor (default 20).

- home_advantage:

  Numeric. Home advantage in Elo points (default 65).
