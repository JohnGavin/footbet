...
Uses brms [`predict()`](https://rdrr.io/r/stats/predict.html) in batch
(not per-match) for speed, then converts expected goals to 1x2
probabilities via Poisson convolution.

## Usage

``` r
predict_matches_brms(model, matches_df, max_goals = 8L)
```
...
- matches_df:
  ...
  A tibble with `home_team` and `away_team` columns.

- max_goals:

  Integer. Maximum goals in score matrix (default 8).

## Value

A tibble with `match_id` (if present), `pred_h`, `pred_d`, `pred_a`,
