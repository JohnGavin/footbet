...
Applies Shin devig to 1X2 odds and power devig to over/under odds for
each match in the odds tibble.
...
- odds_df:
  ...
  with columns `psh`, `psd`, `psa`, `p_over25`, `p_under25`.

## Value

A tibble with `match_id` and devigged probability columns.
