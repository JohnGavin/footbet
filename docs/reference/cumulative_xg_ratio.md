...
``` r
cumulative_xg_ratio(matches_df)
```
...
- matches_df:
  ...
  A tibble with `match_date`, `season`, `home_team`, `away_team`,
  `home_xg`, `away_xg`.

## Value

A tibble with columns `team`, `match_date`, `season`, `cum_xg_for`,
`cum_xg_against`, `xg_ratio`.
