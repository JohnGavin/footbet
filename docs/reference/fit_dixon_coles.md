...
Wraps
[`goalmodel::goalmodel()`](https://rdrr.io/pkg/goalmodel/man/goalmodel.html)
with the Dixon-Coles low-score correction enabled and exponential
time-decay weights.

## Usage

``` r
fit_dixon_coles(matches_df, xi = 0.003)
```
...
- matches_df:
  ...
  A tibble with `home_team`, `away_team`, `fthg`, `ftag`, `match_date`.

- xi:

  Numeric. Time-decay rate parameter (default 0.003). Higher values
  discount older matches more.
