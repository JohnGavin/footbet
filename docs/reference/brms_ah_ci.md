...
Uses posterior draws to compute a distribution of P(cover) for each
match, enabling CI-based bet filtering.
...
- matches_df:
  ...
  Tibble with `match_id`, `home_team`, `away_team`.

- odds_df:

  Tibble with `match_id`, `ah_line`, `pahh`.

- ndraws:

  Integer. Posterior draws per match (default 200).
...
- ci_level:
  ...
  Numeric. Credible interval level (default 0.90).

## Value

Tibble with `match_id`, `p_cover_mean`, `p_cover_lower`,
