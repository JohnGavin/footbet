...
Creates a "long" dataset with one row per team per match, suitable for
GLM/GLMM Poisson regression.

## Usage

``` r
matches_to_long(matches_df)
```
...
A tibble with columns: `match_id`, `match_date`, `season`,
