...
specified date. Standard points system: 3 for win, 1 for draw, 0 for
loss.

## Usage

``` r
league_table(matches_df, as_of_date, league_code = NULL, season = NULL)
```
...
- as_of_date:
  ...
  Date. Calculate table as of this date.

- league_code:

  Character. Optional league filter (default NULL = all).

- season:

  Character. Optional season filter (default NULL = all).

## Value

A tibble with league table: `position`, `team`, `played`, `won`,
