...
Convenience function to compute rolling xG, cumulative xG ratio, and xG
overperformance in one call.

## Usage

``` r
compute_xg_features(matches_df, rolling_window = 5L, overperf_window = 10L)
```
...
- matches_df:
  ...
  A tibble with goals and xG columns.

- rolling_window:

  Integer. Window for rolling averages (default 5).

- overperf_window:

  Integer. Window for overperformance (default 10).

## Value

A tibble with all xG features joined.
