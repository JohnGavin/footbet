...
Adds `home_rest_days`, `away_rest_days`, and `rest_diff` columns. Rest
days = days since team's most recent match before the current one.

## Usage

``` r
compute_rest_days(matches_df)
```
...
The input tibble with three new columns added.
