...
Computes composite form scores for both home and away teams, plus the
form advantage (home score - away score).

## Usage

``` r
add_form_scores(matches_df, ...)
```
...
- matches_df:
  ...
  A tibble with rolling features and Elo ratings. Expected columns:
  `home_rolling_gf`, `away_rolling_gf`, `home_elo`, `away_elo`.
  Optional: `home_rolling_xg_for`, `away_rolling_xg_for`.
...
The input tibble with `home_form_score`, `away_form_score`,
`form_advantage` columns.

## Details

Requires rolling features to be pre-computed via
