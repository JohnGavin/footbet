...
Applies empirical Bayes shrinkage to team attack/defense ratings, useful
when some teams have few matches.

## Usage

``` r
shrink_team_strength(team_stats)
```
...
- team_stats:
  ...
  A tibble with `team`, `n_matches`, `attack`, `defense`.

## Value

The input tibble with `attack_shrunk`, `defense_shrunk` columns.
