...
For each team-matchday, takes the last K residuals (from that team's
perspective — sign-flipped for away matches) and applies exponential
decay weights.

## Usage

``` r
oagd_form(data_with_resid, K = 4L, half_life = 2)
```
...
- K:
  ...
  Integer. Number of past matches to use (default 4).

- half_life:

  Numeric. Half-life in matches for exponential weighting (default 2).

## Value

A tibble with `team`, `matchday`, `form`.
