...
metric. xG+xAG is more predictive than xG alone as it captures build-up
play quality.

## Usage

``` r
compute_xg_xag_composite(matches_df, window = 5L)
```
...
- matches_df:
  ...
  A tibble with xG and xAG columns.

- window:

  Integer. Rolling window (default 5).

## Value

A tibble with xG+xAG composite features.
