...
``` r
rolling_progressive(matches_df, window = 5L)
```
...
- matches_df:
  ...
  A tibble with `team`, `match_date`, plus FBref passing columns: `prgp`
  (progressive passes), `prgc` (progressive carries), `ppa` (passes into
  penalty area).
...
A tibble with `team`, `match_date`, and rolling averages.
