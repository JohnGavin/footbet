...
``` r
kalman_update(state, P, observation, H, Q, R)
```
...
- state:
  ...
  Numeric vector. Current state estimate.

- P:

  Numeric matrix. State covariance.

- observation:

  Numeric. Observed value (xG or goals).

- H:

  Numeric vector. Observation model (maps state to observation).

- Q:

  Numeric matrix. Process noise covariance.
...
A list with `state`, `P` (updated covariance), `innovation`.
