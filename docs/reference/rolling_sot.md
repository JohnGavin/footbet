...
Calculates rolling mean shots on target for and against each team. SoT
ratio = SoT_for / (SoT_for + SoT_against). High discrimination per
ElHabr meta-analytics (0.926).

## Usage

``` r
rolling_sot(matches_df, window = 5L)
```
...
- matches_df:
  ...
  A tibble with `match_date`, `home_team`, `away_team`, `hst`, `ast`
...
A tibble with `team`, `match_date`, `rolling_sot_for`,
