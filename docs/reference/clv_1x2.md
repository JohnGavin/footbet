...
Calculates closing line value for each outcome, returning CLV for the
outcome the model had highest edge on.

## Usage

``` r
clv_1x2(pred_h, pred_d, pred_a, closing_h, closing_d, closing_a)
```
...
- pred_h:
  ...
  Numeric vector. Model's P(Home win).

- pred_d:

  Numeric vector. Model's P(Draw).

- pred_a:

  Numeric vector. Model's P(Away win).

- closing_h:

  Numeric vector. Pinnacle closing odds (Home).

- closing_d:

  Numeric vector. Pinnacle closing odds (Draw).

- closing_a:

  Numeric vector. Pinnacle closing odds (Away).

## Value

A tibble with CLV for each outcome and summary statistics.
