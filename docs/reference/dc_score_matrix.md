...
Constructs a bivariate Poisson score probability matrix with the
Dixon-Coles tau correction applied to low-scoring cells.

## Usage

``` r
dc_score_matrix(lambda1, lambda2, rho = -0.13, max_goals = 8L)
```
...
- rho:
  ...
  Numeric. Correlation parameter (default -0.13).

- max_goals:

  Integer. Maximum goals per side (default 8).

## Value

A (max_goals+1) x (max_goals+1) matrix of probabilities. Rows = home
goals, columns = away goals.
