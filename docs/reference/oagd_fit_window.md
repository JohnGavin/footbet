...
Fits two Poisson GLMMs — one for home goals, one for away goals — to
separate attack and defence strengths (Dixon-Coles style). Uses
[`lme4::glmer()`](https://rdrr.io/pkg/lme4/man/glmer.html) with Poisson
family.

## Usage

``` r
oagd_fit_window(data, target_matchday, window = 8L)
```
...
- data:
  ...
  filtered to one league-season. Must contain `fthg` and `ftag` columns.

- target_matchday:

  Integer. The matchday to fit up to.

- window:

  Integer. Number of matchdays in the rolling window.

## Value

A list with `eta_home` (home goals intercept), `eta_away` (away goals
intercept), `strengths` (tibble with `team`, `attack`, `defence`),
`matchday`, `window`. Returns `NULL` if fit fails.
