...
Calculates rolling mean goals scored and conceded over a window of
recent matches, using only information available before each match.

## Usage

``` r
rolling_goals(matches_df, window = 5L)
```
...
A tibble with columns `team`, `match_date`, `rolling_gf`, `rolling_ga`,
`rolling_gd`.
