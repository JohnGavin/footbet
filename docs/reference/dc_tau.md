...
Multiplies bivariate Poisson probabilities for scorelines 0-0, 1-0, 0-1,
and 1-1 by a correction factor tau that accounts for the negative
correlation between goals in low-scoring matches.

## Usage

``` r
dc_tau(home_goals, away_goals, lambda1, lambda2, rho)
```
...
- home_goals:
  ...
  Integer. Home goals (0 or 1 for correction, else 1).

- away_goals:

  Integer. Away goals (0 or 1 for correction, else 1).

- lambda1:

  Numeric. Home expected goals.

- lambda2:

  Numeric. Away expected goals.

- rho:

  Numeric. Correlation parameter (typically -0.10 to -0.15).

## Value

Numeric. Multiplicative correction factor.
