...
Devig the average bookmaker odds (AvgH/D/A from football-data.co.uk) to
produce fair probabilities. This is the "wisdom of the crowd" baseline —
if no model beats this, models add negative value.

## Usage

``` r
consensus_implied(odds_df)
```
...
- odds_df:
  ...
  A tibble with `match_id`, `avg_h`, `avg_d`, `avg_a`.

## Value

A tibble with `match_id`, `consensus_h`, `consensus_d`, `consensus_a`.
