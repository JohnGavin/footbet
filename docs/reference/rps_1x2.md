...
Better than Brier for ordered categories (H \> D \> A is ordered by goal
difference).

## Usage

``` r
rps_1x2(prob_h, prob_d, prob_a, actual)
```
...
- actual:
  ...
  Character vector ("H", "D", or "A").

## Value

Numeric. Mean RPS (lower is better).
