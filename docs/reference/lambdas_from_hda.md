...
When a model only outputs P(H), P(D), P(A) and no lambdas, this
estimates lambda_home and lambda_away using the probability ratio and a
fixed total-goals assumption.

## Usage

``` r
lambdas_from_hda(ph, pa, total = 2.7)
```
...
- total:
  ...
  Numeric. Assumed total goals per match (default 2.7).

## Value

A list with `lambda_home` and `lambda_away`, or NULL if inputs are
invalid.
