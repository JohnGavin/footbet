...
- shots_df:
  ...
  Understat shot data with `match_id`, `h_a`, `xG`, `result`,
  `situation`, `home_team`, `away_team`, `date`.

## Value

A tibble with one row per team per match date, columns: `team`,
`match_date`, `n_shots`, `xg_total`, `xg_per_shot`, `big_chances` (xG \>
0.3), `open_play_xg_pct`, `goals_from_shots`.
