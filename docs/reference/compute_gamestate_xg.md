...
Filters xG by match situation: tied (0-0 equivalent pressure), close (1
goal difference), or lopsided (2+ goals). xG from lopsided situations
has near-zero predictive value as teams change tactics.

## Usage

``` r
compute_gamestate_xg(matches_df, window = 5L)
```
...
- matches_df:
  ...
  A tibble with xG and goal columns.

- window:

  Integer. Rolling window for averaging (default 5).

## Value

A tibble with gamestate-filtered xG features.
