...
- long_df:
  ...
  Long-format match data.

- matches_df:

  Wide-format match data with outcomes.

- train_months:

  Integer. Months of training data per fold.

- test_months:

  Integer. Months of test data per fold.

- iter:

  Integer. MCMC iterations (reduce for faster CV).
...
- cores:
  ...
  Integer. Cores for parallel.

## Value

A tibble with per-fold evaluation metrics.
