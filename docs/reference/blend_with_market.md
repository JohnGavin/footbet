...
probabilities. The optimal weight is found by minimising log-loss on a
calibration set.
...
- actuals:
  ...
  A tibble with `match_id`, `ftr`.

- weights:

  Numeric vector of blend weights to try (default seq(0, 1, 0.05)).

## Value

A list with `best_weight` and `blended` tibble.
