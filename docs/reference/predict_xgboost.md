...
Generates predictions from a fitted XGBoost model. For classification
returns probabilities; for regression returns values.
...
- new_data:
  ...
  A tibble with the same feature columns as training data.

## Value

For multi-class: a tibble with `prob_h`, `prob_d`, `prob_a`. For
regression: a numeric vector of predicted values.
