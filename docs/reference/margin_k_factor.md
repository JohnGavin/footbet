...
Downweights blowouts for Elo updates. Close matches (1-goal margin) get
full weight; large margins are downweighted using inverse variance.
Based on COOPER methodology (Nate Silver).

## Usage

``` r
margin_k_factor(home_goals, away_goals)
```
...
- home_goals:
  ...
  Integer. Home team goals.

- away_goals:

  Integer. Away team goals.
