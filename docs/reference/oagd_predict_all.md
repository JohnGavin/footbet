...
For each match, looks up team strengths from the prior matchday's fit
and form signals, then predicts via
[`oagd_predict_match()`](https://johngavin.github.io/footbet/reference/oagd_predict_match.md).

## Usage

``` r
oagd_predict_all(data, fits, form_tbl, beta = 0.3, avg_total_goals = 2.7)
```
...
- avg_total_goals:
  ...
  Numeric. League average total goals.

## Value

`data` with added columns: `pred_h`, `pred_d`, `pred_a`, `mu_pred`,
