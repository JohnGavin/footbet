...
Uses [`stats::predict()`](https://rdrr.io/r/stats/predict.html) to get
expected goals (lambda) for each team, then builds a score matrix and
derives 1X2, O/U, and AH probabilities.

## Usage

``` r
predict_glm(model, home_team, away_team, max_goals = 7L)
```
...
A list with `lambda_home`, `lambda_away`, `score_mat`, `probs_1x2`,
`probs_ou25`, `probs_ah05`.
