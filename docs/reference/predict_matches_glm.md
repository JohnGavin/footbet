...
to each row and returns a tibble of predictions.

## Usage

``` r
predict_matches_glm(model, matches_df)
```
...
- matches_df:
  ...
  A tibble with `match_id`, `home_team`, `away_team`.

## Value

A tibble with `match_id`, `pred_h`, `pred_d`, `pred_a`, `pred_over25`,
`pred_under25`.
