...
Returns the number of days since the team's previous match, using only
information available before the specified date.

## Usage

``` r
rest_days(matches_df, team, as_of_date)
```
...
- matches_df:
  ...
  A tibble with `match_date`, `home_team`, `away_team`.
...
- as_of_date:
  ...
  Date. Calculate rest days as of this date.

## Value

Integer. Days since last match, or NA if no previous match.
