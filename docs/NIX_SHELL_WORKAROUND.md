...
The football project has a nested nix-shell conflict: - Current session
is in an **impure nix-shell** from a different project
(roxy.shinylive) - Running `nix-shell default.nix` from within this
shell causes segfaults - Project packages like `goalmodel` are in the
nix store but not in
[`.libPaths()`](https://rdrr.io/r/base/libPaths.html) - Running a new
nix-shell requires building 71 derivations (time-consuming)
...
This prepends football project’s nix package paths to
[`.libPaths()`](https://rdrr.io/r/base/libPaths.html) without spawning a
nested shell.
...
1.  Started session in a different nix-shell (likely via
    `~/docs_gh/rix.setup/default.sh`)
2.  That shell’s R package paths took precedence
3.  Attempting to spawn nested nix-shell causes segfaults
4.  The goalmodel package was built and exists, but isn’t in current
    [`.libPaths()`](https://rdrr.io/r/base/libPaths.html)

## Prevention

- Always verify `IN_NIX_SHELL` and
  [`.libPaths()`](https://rdrr.io/r/base/libPaths.html) at session start
